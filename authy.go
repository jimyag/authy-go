...
// New creates a new Authy API client.
func New() (Client, error) {
	...
	return Client{
		httpCli:    http.Client{},
		UserAgent:  "authy-go (https://github.com/jimyag/authy-go)",
		APIKey:     apiKey,
		nonce:      nonce,
		DeviceApp:  "Authy client for go",
		DeviceName: fmt.Sprintf("Authy client for go on %s", hostname),
	}, nil
}

func NewWithDeviceApp(deviceApp, deviceName string) (Client, error) {
	cli, err := New()
	if err != nil {
		return Client{}, err
	}
...
func (c Client) doRequest(ctx context.Context, method, url string, body io.Reader, dest any) error {
